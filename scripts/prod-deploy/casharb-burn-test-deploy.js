...
async function main () {
  ...
  const CashArb = await ethers.getContractFactory("ArbBurnDebug", impersonateMyself)
  ...
  await impersonateMyself.sendTransaction({
    ...
    let toSwap = 1000000000; // 1k

    console.log(`Swapping ${toSwap/10**6} USDC. Expected output from a swap: ${toSwap * priceUSDCInCASH} CASH (${toSwap * priceUSDCInCASH / 10**18})`)
    tx = await cashArb.work(toSwap); //1k
    await tx.wait()
