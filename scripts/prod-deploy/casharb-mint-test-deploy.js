...
async function main () {
  ...
  const CashArb = await ethers.getContractFactory("ArbMintDebug", impersonateMyself)
  ...
  await impersonateMyself.sendTransaction({
    ...
    for (toSwap of [
      ...
      console.log(`Testing arb with ${Number(toSwap)/10**6} USDC (${toSwap})`)
      let profit = await cashArb.callStatic.work(toSwap); //1k
