...
async function main () {
  ...
  const targetCommunityFee = 150; // 15%
  for (const poolAddress of poolAddresses) {
    ...
    const [_price, _tick, _lastFee, _pluginConfig, communityFee, _unlocked] = await pool.read.globalState();
    if (communityFee != targetCommunityFee) {
      console.log(poolAddress, "communityFee", communityFee, "targetCommunityFee", targetCommunityFee);
      const hash = await pool.write.setCommunityFee([targetCommunityFee,]);
      console.log(poolAddress, "txHash", hash);
