...
const gaugePEPECASH = {"address": "0x152a4fCfb89590C7952EbF48b743681b849e9F7a"}
...
async function main () {
    ...
    const gaugeFactory = await ethers.getContractAt("GaugeFactoryV2_CL", gaugeFactoryCL.address)
    tx = await gaugeFactory.updateoRetroFor(gaugePEPECASH.address);
    await tx.wait()
    tx = await gaugeFactory.updateFeeHandlerFor(gaugePEPECASH.address);
    await tx.wait()
}
