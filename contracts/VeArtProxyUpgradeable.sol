...
contract VeArtProxyUpgradeable is IVeArtProxy, OwnableUpgradeable {
    ...
    /*function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) external pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "Retro locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function _tokenURI(
        ...
        output = string(
            abi.encodePacked(
                ...
                '</text> <text font-size="80px" fill="white" y="1560" x="16" dominant-baseline="middle" class="label" > RETRO Locked: </text> <text font-size="80px" y="1560" fill="white" x="1870" dominant-baseline="middle" text-anchor="end" class="amount" >',
                decimalString(_value / 1e16, 2, false),
                '</text> <text font-size="80px" fill="white" y="1680" x="16" dominant-baseline="middle" class="label"> veRETRO Power: </text> <text font-size="80px" fill="white" y="1680" x="1870" dominant-baseline="middle" text-anchor="end" class="amount">',
                ...
                ' days </text> <text fill="white" y="1858" x="50%" font-size="45px" dominant-baseline="middle" text-anchor="middle" class="app" > Retro.Finance </text></svg>'
            )
        );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "lock #',
                        decimalString(_tokenId, 0, false),
                        '", "description": "Retro locks, can be used to boost gauge yields, vote on token emissions, and receive bribes", "image": "data:image/svg+xml;base64,',
