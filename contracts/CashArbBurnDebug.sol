...
contract ArbBurnDebug is IUniswapV3FlashCallback, Ownable {
    ...
    function work(uint256 amountIn) public onlyOwner returns(uint256 profit) {
        ...
        console.log("making flash loan for ", amountIn, " USDC");
        loanPool.flash(
            address(this),
    ...
    function uniswapV3FlashCallback( // Called after we borrow needed amount
        ...
        console.log("[uniswapV3FlashCallback] Have to return ", remainingDebt, " USDC");
        
        console.log("[uniswapV3FlashCallback] Swapping ", decoded.borrowedAmount, " USDC");
        // Swap borrowed USDC to CASH
        uint256 amountCASH = v3Swap(retroRouter, USDC, CASH, decoded.borrowedAmount);
        console.log("[uniswapV3FlashCallback] received ", amountCASH, " CASH");

        // Burn Cash
        console.log("[uniswapV3FlashCallback] Redeeming ", amountCASH, " CASH");
        ...
        console.log("[uniswapV3FlashCallback] Received from redeem ", balanceUSDC, " USDC");
        console.log("[uniswapV3FlashCallback] Received from redeem ", balanceDAI, " DAI");
        console.log("[uniswapV3FlashCallback] Received from redeem ", balanceUSDT, " USDT");

        console.log("[uniswapV3FlashCallback] Total amount: ", balanceUSDC + balanceDAI/10**12 + balanceUSDT );

        console.log("Swapping", balanceDAI, "DAI to USDC");
        uint256 balanceUSDCBefore = balanceUSDC;
        v3Swap(uniswapRouter, DAI, USDC, balanceDAI);
        balanceUSDC = IERC20(USDC).balanceOf(address(this));
        console.log("[uniswapV3FlashCallback] received ", balanceUSDC - balanceUSDCBefore, " USDC");
        console.log("[uniswapV3FlashCallback] received exchange rate: ", (balanceUSDC - balanceUSDCBefore) *10**10 /  (balanceDAI/10**12), "/ 10**10");

        console.log("Swapping" , balanceUSDT, "USDT to USDC");
        balanceUSDCBefore = balanceUSDC;
        v3Swap(uniswapRouter, USDT, USDC, balanceUSDT);
        balanceUSDC = IERC20(USDC).balanceOf(address(this));
        console.log("[uniswapV3FlashCallback] received ", balanceUSDC - balanceUSDCBefore, " USDC");
        console.log("[uniswapV3FlashCallback] received exchange rate: ", (balanceUSDC - balanceUSDCBefore) *10**10 / (balanceUSDT), "/ 10**10" );

        console.log("[uniswapV3FlashCallback] Total balance USDC ", balanceUSDC, " USDC");
