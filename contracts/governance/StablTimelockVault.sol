...
contract StablTimelockVault {
    ...
    event SignalUpgradeTo(address newImplementation);
    event UpgradeTo(address newImplementation);
    ...
    function signalUpgradeTo(address _newImplementation) external onlyEcosystem {
        bytes32 action = keccak256(abi.encodePacked("upgradeTo", _newImplementation));
        _setPendingAction(action, true); // buffer for critical actions

        emit SignalUpgradeTo(_newImplementation);
    }

    function upgradeTo(address _newImplementation) external onlyEcosystem {
        bytes32 action = keccak256(abi.encodePacked("upgradeTo", _newImplementation));
        _validateAction(action);
        _clearAction(action);

        IVault(cashVault).upgradeTo(_newImplementation);

        emit UpgradeTo(_newImplementation);
    }
