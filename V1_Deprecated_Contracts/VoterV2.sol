...
contract VoterV2 is IVoter, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (msg.sender != governor) { // gov can create for any pool, even non-Retro pairs
            require(isPair, "!_pool");
            require(isWhitelisted[tokenA] && isWhitelisted[tokenB], "!whitelisted");
        }

        string memory _type =  string.concat("Retro LP Fees: ", IERC20(_pool).symbol() );
        address _internal_bribe = IBribeFactory(bribefactory).createBribe(owner(), tokenA, tokenB, _type);

        _type = string.concat("Retro Bribes: ", IERC20(_pool).symbol() );
